    ...
    if (lpBits)
      {
        ...
        lpBits+=(
				// (200 * 1)+0 *(200-1)
				(imageWidth*SamplePerPixel)+Align)
				*(imageLength-1);
...
//<=> BGR
        ...
        {     
            nrow = ( ( (row + RowsPerStrip) > imageLength) 
				? imageLength - row	// end of image
				: RowsPerStrip );	// read n rows
            ...
            {  
				// Iterate thru each row in this strip
				for (l = 0; l < nrow; l++) 
				{
					if (SamplePerPixel  == 3)
                    {	for (i=0;i< (int) (imageWidth);i++)
                        {
							lpBits[i*SamplePerPixel+0]=buf[l*LineSize+i*SamplePerPixel+2]; 
                            lpBits[i*SamplePerPixel+1]=buf[l*LineSize+i*SamplePerPixel+1];
                            lpBits[i*SamplePerPixel+2]=buf[l*LineSize+i*SamplePerPixel+0];
                        }
					}
                    else
					{
...
HDIB CreateDIB(DWORD dwWidth, DWORD dwHeight, WORD wBitCount)
{
   ...
   bi.biSizeImage = (dwWidth*dwHeight*wBitCount)/8; //0; // 0's here mean "default"
