...
static	uint32 ReadDirectory(int, FILE *, unsigned, uint32);
static	void ReadError(char*);
static	void Error(const char*, ...);
static	void Fatal(const char*, ...);
static	void dump(int fd, FILE *ostr, uint32 diroff);
...
#include <direct.h>		//	Win 95, Win NT, Win32s, 68K, PMac

int DumpOneFile( char *pfSpec )
{
	int one = 1, fd;
	uint32 diroff = (uint32) 0;
	FILE *ofile;

	// is this machine bigendian ?
	bigendian = (*(char *)&one == 0);

	{
		char szd[200];
		_getcwd( szd, sizeof( szd ));
	}

	fd = open(pfSpec, O_RDONLY|O_BINARY, 0);
	if (fd < 0) {
		//perror(argv[0]);
		return (-1);
	}
	swabflag = 0;
	ofile = fopen( "~TIFINFO.TXT", "wt" );
	if ( !ofile )
		return( -1 );

	dump(fd, ofile, diroff);
	close(fd);
	fclose( ofile );
...
static void
dump(int fd, FILE *ostr, uint32 diroff)
{
	unsigned i;
	...
	fprintf( ostr, "Magic: %#x <%s-endian> Version: %#x\n",
	    hdr.tiff_magic,
	...
	for (i = 0; diroff != 0; i++) {
		if (i > 0)
			fputc('\n', ostr );
		diroff = ReadDirectory(fd, ostr, i, diroff);
	}
}
 ...
 */
static uint32
ReadDirectory(int fd, FILE *ostr, unsigned ix, uint32 off)
{
	...
	fprintf( ostr, "Directory %u: offset %lu (%#lx) next %lu (%#lx)\n", ix,
	...
	for (dp = dir, n = dircount; n > 0; n--, dp++) {
		...
		PrintTag( ostr, dp->tdir_tag);
		fputc(' ', ostr );
		PrintType( ostr, dp->tdir_type);
		fputc(' ', ostr );
		fprintf( ostr, "%lu<", (unsigned long) dp->tdir_count);
		...
		if (space <= 4) {
			...
			case TIFF_BYTE:
				PrintByte( ostr, bytefmt, dp);
				break;
			case TIFF_SBYTE:
				PrintByte( ostr, sbytefmt, dp);
				break;
			case TIFF_SHORT:
				PrintShort( ostr, shortfmt, dp);
				break;
			case TIFF_SSHORT:
				PrintShort( ostr, sshortfmt, dp);
				break;
			case TIFF_LONG:
				PrintLong( ostr, longfmt, dp);
				break;
			case TIFF_SLONG:
				PrintLong( ostr, slongfmt, dp);
				break;
			}
		} else {
...
//	TCHAR szMsg[200];
	...
	fprintf(fd, "%s: ", g_pzTiffSpec );
	vfprintf(fd, fmt, ap);
	fprintf(fd, ".\n");
}
static void
vsError( const char* fmt, va_list ap)
{
	TCHAR szMsg[200];
	//TCHAR szCaption[100];
	//g_Msg << g_curfile << ": ";
	...
	sprintf( szMsg, "%s: ", g_pzTiffSpec );
	vsprintf( &szMsg[strlen(szMsg)], fmt, ap); 
	// ( char *buffer, const char *format, va_list argptr ); 
	// sprintf(fd, ".\n");
	MessageBox( g_hWndMain, szMsg, APPNAME, MB_OK|MB_ICONHAND );
}
...
Fatal(const char* fmt, ...)
{
	...
	vsError( fmt, ap);
	va_end(ap);
	PostMessage( g_hWndMain, WM_CLOSE, 0, 0L );
}
