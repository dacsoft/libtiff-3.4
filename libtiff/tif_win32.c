...
{
	DWORD dwSizeRead;
	if (!ReadFile( (HANDLE) fd, buf, size, &dwSizeRead, NULL))
		return(0);
...
{
	DWORD dwSizeWritten;
	if (!WriteFile( (HANDLE) fd, buf, size, &dwSizeWritten, NULL))
		return(0);
...
_tiffSeekProc(thandle_t fd, toff_t off, int whence)
{
	...
	return ((toff_t)SetFilePointer((HANDLE) fd, off, NULL, dwMoveMethod));
}
...
{
	return ((toff_t)GetFileSize((HANDLE) fd, NULL));
}
...
_tiffMapProc(thandle_t fd, tdata_t* pbase, toff_t* psize)
{
	...
	if ((*phMapFile = CreateFileMapping((HANDLE) fd, NULL, PAGE_READONLY, 0, size, NULL))
			== NULL)
		return(0);
...
TIFFOpen(const char* name, const char* mode)
{
	...
	if (fd == (thandle_t) INVALID_HANDLE_VALUE) {
